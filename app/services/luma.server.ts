...
const shapeLumaEvent = (event: LumaEvent): LumaEvent => ({
  api_id: event.api_id,
...
const shapeLumaGuests = (guests: LumaGuest[]): LumaGuest[] =>
  guests.map((guest) => ({
    api_id: guest.api_id,
...
export const createLumaClient = ({ isMock } = { isMock: false }) => {
  ...
  const getEventInfo = async (url: string) => {
    if (isMock) {
      const html = await fs.readFile('./json/NEXT_DATA.json', 'utf-8')
      const pageData = JSON.parse(html) as {
        props: { pageProps: { initialData: { data: { event: LumaEvent } } } }
      }
    ...
    // イベント情報を取得
    const nextData = jsdom.window.document.getElementById('__NEXT_DATA__')
    if (!nextData || nextData.textContent === null) {
      throw new Error('No __NEXT_DATA__ found')
    }

    const pageData = JSON.parse(nextData.textContent) as {
      props: { pageProps: { initialData: { data: { event: LumaEvent } } } }
    }
    return shapeLumaEvent(pageData.props.pageProps.initialData.data.event)
  }
  ...
  const signIn = async ({
    ...
    sessionKey = cookie.match(/luma\.auth-session-key=([^;]+)/)?.[1] ?? null
    return sessionKey
  }
  ...
  const listEventGuests = async (eventId: string) => {
    if (isMock) {
      const json = await fs.readFile('./json/event-guests.json', 'utf-8')
      return shapeLumaGuests(JSON.parse(json)['entries'] as LumaGuest[])
    }
    ...
    const ret = await fetch(
      ...
      {
        method: 'GET',
        headers: { Cookie: `luma.auth-session-key=${sessionKey}` },
      },
    )

    const guests = (await ret.json()).entries as LumaGuest[]
